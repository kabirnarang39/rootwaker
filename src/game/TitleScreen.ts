...
export class TitleScreen {
  ...
  private syncLinkBtn: HTMLButtonElement;
  ...
  constructor(container: HTMLElement) {
    ...
    this.root.innerHTML = `
      <style>
        ...
        .rw-title-sync-link {
          ...
          font-size: 11px; color: var(--parchment, #eef2e6); opacity: 0.55;
          transition: opacity 140ms ease;
        }
        .rw-title-sync-link:hover { opacity: 0.9; }
      </style>
      <div class="rw-title-panel">
        ...
        <div class="rw-title-actions">
          ...
          <button class="rw-title-sync-link" type="button">Move progress to another browser</button>
        </div>
      </div>
    `;
    ...
    this.syncLinkBtn = this.root.querySelector('.rw-title-sync-link')!;
    ...
    // Dynamically imported — same code-splitting idiom Game.ts already uses for the duel flow
    // (ChallengeGate/DuelSession): trystero and the save-sync handshake logic have no business
    // in the FIRST bundle every player downloads before they've even chosen to use this feature.
    this.syncLinkBtn.addEventListener('click', () => {
      void import('../multiplayer/SaveSyncPanel').then(({ SaveSyncPanel }) => {
        new SaveSyncPanel(this.root.parentElement ?? document.body);
      });
    });
  }
