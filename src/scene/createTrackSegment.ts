...
export const DEFAULT_PALETTE: TrackPalette = {
  barkColor: 0x2c1e14,
  barkDark: 0x1a120c,
...
function buildTree(rand: () => number, palette: TrackPalette): THREE.Group {
  const tree = new THREE.Group();
  const trunkHeight = 3.5 + rand() * 2.5;
  const trunkRadius = 0.22 + rand() * 0.12;

  const trunkMat = new THREE.MeshStandardMaterial({
    color: palette.barkColor,
    flatShading: true,
    roughness: 0.95,
    metalness: 0.02,
  });
  ...
  const foliageMat = new THREE.MeshStandardMaterial({
    color: palette.foliageColors[Math.floor(rand() * palette.foliageColors.length)],
    flatShading: true,
    roughness: 0.85,
  });
...
function buildGlowPlant(
...
): { group: THREE.Group; mat: THREE.ShaderMaterial } {
  const mat = createGlowMaterial({
    color: palette.moteGlow,
    rimColor: 0xffffff,
    intensity: 0.6,
    fresnelPower: 1.8,
  ...
  const light = new THREE.PointLight(palette.moteGlow, 0.25, 2, 2);
...
export function createTrackSegment(seed = 1, palette: TrackPalette = DEFAULT_PALETTE): TrackSegment {
  const rand = mulberry32(seed);
  const group = new THREE.Group();
  group.name = 'track-segment';

  const floorMat = new THREE.MeshStandardMaterial({ color: palette.floorColor, roughness: 1, flatShading: true });
  ...
  for (let i = 0; i < treeCount; i++) {
    ...
    const tree = buildTree(rand, palette);
  ...
  for (let i = 0; i < plantCount; i++) {
    ...
    const { group: plant, mat } = buildGlowPlant(rand, palette);
